...
class HKVKerneloptBinding : public IBinding<K, V> {
private:
...
template<typename K, typename V>
HKVKerneloptBinding<K, V>::HKVKerneloptBinding() 
...
template<typename K, typename V>
bool HKVKerneloptBinding<K, V>::initialize(const InitConfig& config) {
...
template<typename K, typename V>
void HKVKerneloptBinding<K, V>::cleanup() {
    table_.reset();
    std::cout << "HKV binding cleaned up" << std::endl;
}

template<typename K, typename V>
void HKVKerneloptBinding<K, V>::multiset_for_loading(uint32_t batch_size,
                               const K* d_keys,
                               const V* d_values,
                               const CallContext& ctx) {
...
template<typename K, typename V>
void HKVKerneloptBinding<K, V>::multiset(uint32_t batch_size,
                               const K* d_keys,
                               const V* d_values,
                               const CallContext& ctx) {
...
template<typename K, typename V>
void HKVKerneloptBinding<K, V>::multiput(uint32_t batch_size,
                               const K* d_keys,
                               const V* d_values,
                               const CallContext& ctx) {
...
template<typename K, typename V>
void HKVKerneloptBinding<K, V>::multiget(uint32_t batch_size,
                               const K* d_keys,
                               V* d_values_out,
                               bool* d_found,
                               const CallContext& ctx) {
...
template<typename K, typename V>
void HKVKerneloptBinding<K, V>::insert_and_evict(uint32_t batch_size,
                               const K* d_keys,
                               const V* d_values,
                               const CallContext& ctx) {
...
template<typename K, typename V>
void HKVKerneloptBinding<K, V>::accum_or_assign(uint32_t batch_size,
    const K* d_keys,
    const V* d_values,
    const CallContext& ctx) {
...
template<typename K, typename V>
void HKVKerneloptBinding<K, V>::find(uint32_t batch_size,
                               const K* d_keys,
                               V* d_values_out,
                               bool* d_found,
                               const CallContext& ctx) {
...
template<typename K, typename V>
void HKVKerneloptBinding<K, V>::find_or_insert(uint32_t batch_size,
                               const K* d_keys,
                               V* d_values_out,
                               bool* d_found,
                               const CallContext& ctx) {
...
using HKVKerneloptBinding_u64d = HKVKerneloptBinding<uint64_t, double>;
REGISTER_CUDA_BINDING(uint64_t, double, HKVKerneloptBinding_u64d, "hkv_kernelopt");

using HKVKerneloptBinding_u64f = HKVKerneloptBinding<uint64_t, float>;
REGISTER_CUDA_BINDING(uint64_t, float, HKVKerneloptBinding_u64f, "hkv_kernelopt");